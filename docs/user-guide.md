...
3. With the **Perspective Match** sidebar tab selected, **Ctrl+Alt+NumPad 1–9** jumps to the Nth match (name-sorted). **Ctrl+Alt+←/↑** and **Ctrl+Alt+→/↓** step to the previous or next match (wraps). **Ctrl+Alt+Shift+←/→** steps through the last 10 matches you selected (back/forward, wraps). Any Draw / Pick Origin / PP / Landmark tool is cancelled first. Pressing the shortcut for the match that is already active keeps the current zoom and pan. In that match's **camera view**, **Ctrl+Cmd+A** (macOS; **Ctrl+Win+A** on Windows/Linux) starts **Pick in Active Match** for the selected landmark.
