...
Each match is a collection with a root Empty that owns the session data, plus a child camera. **Hide Origin Empty** (Sync section, under Landmark Empties) is on for new matches and stored per match: it hides that Origin Empty in the viewport while the camera and collection stay visible. Hiding the Origin in the Outliner checks the box the next time you open that match. While **Solve Sync** or **Investigate Problems** is running, that row shows the current stage and elapsed time; later this-match RMSE sits in a collapsed **Info** section below **Hide Origin Empty**. After **Solve Sync**, the **Enable sync for current match** row shows **Synced** or **Not synced** for the active match in that last run. **This Camera** (same expanded row as Perspective / Axis) chooses how Solve Sync uses a non-anchor match: **Solve** (default) may move this camera and lets its 2D picks move 3D landmarks; **Lock Pose** keeps the current root location, rotation, and scale while those picks still constrain landmarks and the other cameras; **Fit Only** only places this camera against 3D from the other matches. The row is disabled on the Anchor, which is always fixed.
...
3. With the **Perspective Match** sidebar tab selected, **Ctrl+Alt+NumPad 1–9** jumps to the Nth match (name-sorted). **Ctrl+Alt+←/↑** and **Ctrl+Alt+→/↓** step to the previous or next match (wraps). **Ctrl+Alt+Shift+←/→** steps through the last 10 matches you selected (back/forward, wraps). **Pick Landmark** stays active when you switch matches by shortcut or **Active Match** dropdown, so you can place the selected landmark on each still without restarting the tool. An unfinished line or drag is discarded on switch. Draw / Pick Origin / PP tools still end. Pressing the shortcut for the match that is already active keeps the current zoom and pan. In that match's **camera view**, **Ctrl+Cmd+A** (macOS; **Ctrl+Win+A** on Windows/Linux) starts **Pick in Active Match** for the selected landmark.
...
Selecting objects in the viewport does **not** change the active match—only New, Rename, Unload, Delete, the dropdown, and the match / history shortcuts do.
...
fits each lens from image picks and supplied constraints. Known 3D is optional;
VP lines are not required. Check each match's
...
required to finish applying it.

Immediately running Solve after a converged independent lens fit should leave
the overlays essentially unchanged. Small numerical changes or further
improvement are possible if the fit stopped early. Both operations consider
points, lines, and supplied constraints; a lower point RMSE alone is not the
criterion for replacing the current solution.

Both operations save a local report automatically. **Open Last Report** shows
that result without solving again, and **Export** saves a permanent HTML copy.
The report separates point error from line error and shows camera coverage,
constraints, and whether the result was applied. Opening or exporting after
input edits marks the old report as outdated. A lower point error alone does
not establish that line alignment or reconstruction accuracy improved.

**Advanced diagnostics → Investigate Problems** runs an optional diagnostic
trial without applying it. Its report does not overwrite the visible solution's
landmark errors. See [Sync matches](sync.md) for solver behavior and constraints.

## Set origin

Use **Pick Origin** to choose a ground point that should become world origin (placement updates automatically after VP solves). The Origin section eye hides the green origin mark on the plate. Clear (X) removes the pick. **Solve Sync** / **Refine Lenses** also auto-set Origin from the first **On Ground** landmark pick when an unlocked match still has none; **Lock Pose** suppresses that placement change. With no ground pick, a world YZ/XZ **Mirror Position → Landmark** can instead place the Anchor's 3D Origin below the reconstructed landmarks after a successful fit; it leaves the 2D Pick Origin marker unset. See [Mirror pairs](sync.md#mirror-pairs) for the conditions. **Ground Slack** (next to Lock Rotation) is how far those On Ground tags may leave Z=0 — leave the default for a boarded floor; 0 for a machined plate. **Known 3D Slack** (beside it) is how far Known 3D points may leave their Empty during Solve Sync — 0 keeps them pinned; a small value lets CAD / mesh error ease toward the picks. A point that is both On Ground and Known 3D uses the tighter slack for Z. The linked Empty is not moved; Landmark Empties show the eased position. **Plane Slack** (below those, beside Mirror Slack) is how far **Is in Plane** landmarks may leave their shared X / Y / Z / Free plane. **Sync Weight** on a landmark (default 1, above Pick in Active Match) makes that feature pull harder in Solve Sync — raise it for a couple of well-placed picks that sit far from the others. The landmark list **Filter** (funnel) shows only this still's picks and that still's overlay miss in pixels; the unfiltered list keeps the all-views RMSE. **Use Known 3D** in Camera still treats those Empties as fixed for the single-still polish. Landmark-reference controls (**Point A/B**, **Is Parallel To**, **Is Mirror Of**, and **On-plane Point**) open Blender's native search field; start typing to filter the eligible names. **Mirror Empty** (with **Local Plane** and **Mirror Plane Slack**) is the shared midline for **Is Mirror Of** point or line pairs — the pair is stored on both landmarks, and the magic wand next to that selector fills the partner when the name ends with left/right. Mirrored points and lines show the mirror icon in the landmark list; a line also keeps its line and parallel-link icons. For a moving reconstructed reference instead, choose **Mirror Position → Landmark**, select an **On-plane Point**, and choose the world plane or an optional orientation object. The plane follows the point inside Sync and point-FOV fitting; the point need not be the object’s center. See [Sync matches](sync.md#mirror-pairs). **Is in Plane** (X / Y / Z / Free plus a #1–#10 bucket) groups landmarks that share a wall, table, or other plane. Bucket choices show the number of assigned point and line landmarks for that plane type, including disabled landmarks, or **(empty)**. With zero Plane Slack, an already supported plane can place a point picked in one Solve or Lock Pose view; the report labels its depth source. See [Shared planes](sync.md#shared-planes).
...
**Mirror Slack** controls position mismatch between paired points or drawn lines: 0 fits exact reflections, while a positive value permits up to that many scene units of point or transverse line offset. Mirrored line directions remain exact. **Mirror Plane Slack** separately lets the shared plane slide along its normal without moving its Empty or live landmark.
...
**Refine Distortion** appears with **Estimate FOV from Landmarks** in the Sync controls. It is off by default. When enabled, an accepted landmark FOV fit is followed by a bounded one-parameter radial-distortion check for each still. FOV, camera pose, and reconstructed geometry remain fixed. The correction is kept only when at least 16 broadly distributed point picks support it on both a fitting subset and a radial-stratified validation subset. Imported Brown–Conrady calibration remains unchanged. Any reported FOV intervals describe the focal-only fit before this postpass.
...
- Initial FOV is manual. Shared point picks across translated views can refine it without VP lines: use Same Lens for a shared correction, or the optional Estimate FOV from Landmarks mode for independent lenses. With world-axis plane or parallel constraints, or a supplied mirror normal, independent fitting can also correct the overall orientation of the cameras and landmarks while keeping the anchor camera center fixed. Point estimates include conditional local uncertainty and can be refused for weak evidence; see [the Sync workflow](sync.md#independent-fov-estimates-from-shared-points-no-vp-lines). GeoCalib automatic FOV/gravity estimation is intentionally omitted.
