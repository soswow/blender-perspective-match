...
| Conservative post-fit distortion | `core/distortion_refine.py` (opt-in division-model polish after accepted independent FOV fitting; focal, pose and geometry stay frozen; radial coverage and validation-pick gates) |
| Shared fixed/free focal final fit | `core/focal_bundle.py` (NumPy joint focal/pose/point/line fit; bounded runtime/size, point depth screen, conditional sensitivity; `MAX_ITERATIONS=400`, `MAX_SECONDS=60`, `MAX_DENSE_JACOBIAN_BYTES` for the bundle); `core/focal_orientation.py` (observable common-rotation basis from point/line world relations; `ORIENTATION_INDEPENDENCE_SINE`; corrected anchor orientation persists in its private calibration with identity root); `core/focal_lines.py` (four-coordinate infinite lines, stroke residuals); `core/derived_lines.py` (validation and exact endpoint materialization for From Points lines; `MIN_DERIVED_LINE_SPAN` rejects coincident geometry); `core/focal_constraints.py` (point plane and shared mirror plane constraints); `core/focal_line_constraints.py` (point-supported line plane position/direction, line/line or line/world-axis parallelism, including endpoint Jacobian dependencies for From Points lines, `LINE_RELATION_DIRECTION_RESIDUAL_PX`, `LINE_RELATION_DIRECTION_HARD_SINE`, `LINE_HARD_PLANE_MIN_TANGENT` and the one remaining-budget `DERIVED_FIXED_DIRECTION_FEASIBILITY_MULTIPLIER` continuation for hard fixed targets; hard-plane directions stay constrained inside pixel residuals and publication). `scene.run_solve_sync` calls `refine_fixed_focals` after initialization; independent `core/lens_refine.py` frees the focal parameters in the same fit. Line planes need one independently located same-bucket member for X/Y/Z or three non-collinear members for Free; fitted points and fixed Known 3D line references can supply support. From Points lines never supply independent plane support or image evidence, and their final geometry is always the segment between their fitted point landmarks. |
| Shared fit scoring and projection | `core/joint_fit_score.py` (`JointFitScorer` freezes current evidence weights for incumbent/candidate comparisons; `supported_joint_request` records excluded cameras, picks and relations); `core/focal_projection.py` (aspect/distortion-aware point derivatives and representation/scale-invariant distorted line sampling); `core/focal_point_priors.py` (hard/soft Known 3D and Ground, including the tighter combined Z leash; `ROTATION_ROUNDOFF_ATOL` validates persisted float32 rotation matrices). |
...
| Point-FOV plane and mirror constraints | `core/focal_constraints.py` (initial-anchor coordinate conversion, axis/Free-plane, exact zero-slack object/live-landmark point mirrors, positive pair-position springs, normal-only Mirror Plane Slack and appropriate baseline scale freedom (live reference keeps global scale free); geometric rows are not independent image picks) |
...
| Provisional independent FOV results | `core/focal_bundle.py` (`FocalFitCandidate`, distinct from accepted calibration); `core/lens_refine.py` propagates the candidate; `scene.apply_lens_refine_result(..., use_candidate=True)` explicitly applies through the existing stale-input and rollback checks. `ui/operators.py` owns one temporary Use Best Fit result, cleared on new lens job, use or file load. |
| Sync input collection, continuation and result ownership | `scene/__init__.py` (`collect_sync_request` reads without preparation; applied evidence/geometry hashes certify a complete `SyncSolutionSeed`; `run_solve_sync` compares common-fit candidates before application; investigation validates ownership without replacing applied errors). `properties/__init__.py` persists seed provenance and applied diagnostics across save/reopen. |
...
| Self-contained HTML sync diagnostics | `ui/sync_report.py` (automatic Solve/Refine reports, applied/trial ownership, separate point/line errors and report provenance; Cytoscape.js in `ui/vendor/` for the camera graph). `ui/operators.py` publishes exact final results without another solve; Open/Export checks staleness, sidebar redraw reads cached state only. |
| RNA | `properties/__init__.py` |

Focal results can change the anchor's private orientation. Apply and assess both
returned calibrations and root similarities; copying only fitted focal lengths
onto the original private poses loses the corrected frame. Synthetic reports
...
The bundle allows `MAX_ITERATIONS=400` within `MAX_SECONDS=60`.
`RELATIVE_COST_TOLERANCE=1e-7` controls convergence after an accepted step,
normalized by `max(objective, 1)`; it is separate from provisional-candidate
eligibility and does not relax physical/noise/uncertainty gates. The archived
weak-axis regression in `tests/test_focal_tolerance.py` checks earlier stopping
against independent withheld geometry and focal-interval truth coverage.
Time exhaustion stops optimization but retains an improved physically checked
candidate; it skips the extra pair-hint search. User cancellation still discards
the endpoint. `tests/test_focal_candidate.py` covers that distinction.
...
Certified applied common fits persist effective point weights in `joint_point_weights` on `SyncSolveResult` / `SyncAppliedDiagnostics`. Reuse these weights only for the same captured evidence; changed inputs recompute them. Old or incompatible weight records are unproven starts. `_apply_sync_solve_result` updates the dependency graph before stamping live-camera provenance.

Mirrored infinite-line priors use `core/focal_lines.py:canonical_line_point` in both the fitter and public scorer, making their score independent of finite helper midpoint placement. Known 3D line midpoints remain unchanged as supporting-plane evidence. Fixed/free endpoint acceptance uses the represented geometry's full current-evidence score, not equality with the optimizer's private-chart cost.
...
| Module | Role |
| --- | --- |
| `constants.py` | `WORLD_AXIS_DIRECTIONS`, `ACCEPT_RMSE_PX`, `RESECT_MISMATCH_CANDIDATE_LIMIT`, `STRETCHED_PIXEL_RATIO`, `GROUND_PLANE_Z_FRACTION`, `GROUND_SLACK_DEFAULT`, `GROUND_Z_RESIDUAL_PX`, `GROUND_Z_HARD_SLACK`, `KNOWN_3D_SLACK_DEFAULT`, `KNOWN_3D_RESIDUAL_PX`, `MIRROR_SLACK_DEFAULT`, `MIRROR_PLANE_RESIDUAL_PX`, `MIRROR_PAIR_HARD_GAP`, `MIRROR_PAIR_RESIDUAL_PX`, `MIRROR_PAIR_SLACK_DEFAULT`, `MIRROR_PAIR_EXACT_RELATIVE_TOLERANCE`, `PLANE_SLACK_DEFAULT`, `PLANE_RESIDUAL_PX`, `PLANE_HARD_SLACK`, `LOG_SCALE_CLIP`, `LINE_FIXED_ANCHOR_MIN`, `LINE_PLANE_MIN_SINE`, `LINE_CONSTRAINT_DIRECTION_TOLERANCE`, `LINE_RECONSTRUCT_TRUNCATE_PX`, `RECOVERED_HUBER_DELTA_PX`, `BA_ACCEPT_RMSE_FLOOR_PX`, `BA_ACCEPT_RMSE_SLACK_PX`, `BA_FREE_LANDMARK_LIMIT`, `SPATIAL_GRID_SIZE`, `SPATIAL_WEIGHT_CLIP`, `RADIAL_WEIGHT_GAIN`, `TRIANGULATION_GN_STEPS`, `TRIANGULATION_ANGLE_WEIGHT_FLOOR`, `TRIANGULATION_PARALLEL_COSINE`, `SYNC_WEIGHT_PROTECT`, `DIAGNOSE_COMMON_MAX_CANDIDATES`, `DIAGNOSE_COMMON_MAX_SECONDS` |
| `types.py` | `SimilarityTransform`, observations, `SyncSolveResult`, complete `SyncSolutionSeed` and `SyncAppliedDiagnostics` |
| `request.py` | Complete `SyncSolveRequest` v6, JSON capture/restore, separate evidence/seed fingerprints; v1–5 migrate without From Points definitions (v1–4 also migrate without a seed) |
...
| `pose.py` | Essential / PnP / IPPE / pairwise register; reuse pair rays and accepted residuals; serial pair jobs when Python has the GIL, ordered workers otherwise; free-ray refinement holds the seed camera-baseline scale gauge; shared-ground seeds from registered location-enabled cameras |
...
**Mirror pair slack:** `mirror_pair_slack` is part of Sync request v7; v1–6 load with exact zero pair slack. It controls point and drawn-line position mismatch, while the older persisted `mirror_slack` remains normal-only Mirror Plane Slack. Zero pair slack is represented exactly throughout the common fit; mirrored drawn-line directions remain exact at positive pair slack too.

## Debugging tools

- `tools/synthetic_sync/sync_continuation.py` — staged generated Refine → Solve → Solve checks with fixed or live mirror and free-scale cases, exact requests/results and independent withheld geometry; generated scenes can save/reopen between stages. Its supplied-project mode never saves the source, checks its hash, and compares applied endpoints with persisted weights and explicit numerical/process caps.
- `tools/synthetic_sync/verify_sync_continuation_blender.py` — zero-solve native seed provenance, changed-input, save/reopen, incumbent objective/support selection, cancellation, and investigation ownership checks.
- `tools/synthetic_sync/joint_fit_gate.py` and `joint_fit_distortion_probe.py` — bounded generic common-fit feature controls with source/runtime archives, exact inputs/results and separate withheld-geometry assessments.
- `tools/synthetic_sync/joint_fit_fullsuite_probe.py` — bounded replay of the public weak-axis, focal recovery and provisional-candidate regressions, retaining exact inner calls and source identity.

- `tools/synthetic_sync/verify_report_ownership_blender.py` — generated zero-solve report publication for Solve, Refine, provisional/refused fits and investigation; checks final live focal calibration, origin adjustment, stale input marking, no browser auto-open, and report I/O failure after a successful apply.

- `tools/debug-sync/benchmark_solvers.py` — bounded read-only Blender Sync/lens timings, complete input/result comparisons, per-inner-call ledgers, source/runtime archives and optional profiles or serial-pair diagnostic control. Keep private captures outside the repo.

- `tools/synthetic_sync/joint_fit_saved_endpoint.py` — replay an archived initialized joint fit without camera registration, retaining frozen pick weights, archived inputs/options, exact sources and separate internal/public endpoint objectives; `--public-wrapper` replays Refine Lenses from the saved startup through one mocked registration call, refuses missing startup cameras and checks the original-weight public objective. Checks hard-plane line geometry after finite segment publication.
- `tools/synthetic_sync/replay_sync_stamp_blender.py` — zero-solve replay of a generated archived Refine result, tracing live-camera drift and solution certification before/after the dependency graph update; never saves a supplied project.
- `tools/synthetic_sync/known_focal_control.py` — one bounded paired fit at the generated oracle's focal lengths with unchanged noisy picks, relations and effective weights; archives both the inner endpoint and wrapper decision so a refusal cannot hide geometry evidence.
- `tools/synthetic_sync/continuation_gauge.py` — zero-solve orientation-rank and allowed world-frame alignment audit; fits any permitted alignment from training geometry before assessing withheld points.
- `tools/synthetic_sync/continuation_accuracy_control.py` — capped oracle-start/noisy and fitted-start/noise-free fixed-focal controls for a generated continuation fixture, with exact source/input/result archives; its read-only frame audit quantifies training-frame sensitivity without treating forbidden rotation as a gauge. See `tools/synthetic_sync/continuation-accuracy-results.md`.
...
- `tools/synthetic_sync/verify_lens_progress.py` — native Blender operator callback/RNA checks for textual independent-FOV activity, correctly scaled ordinary lens-search progress, and Diagnose/Solve Sync sidebar activity without a cursor overlay; no numerical solves.
...
- `tools/synthetic_sync/verify_mirror_origin_blender.py` — generated zero-solve native checks for free vertical mirror-origin placement, projection invariance, ineligible controls, repeat application, save/reopen and Clear Sync.
...
- `tools/debug-sync/probe_focal_startup.py` — read-only diagnostics from saved lens `.inputs.json` / `.startup.json`: bounded OpenCV PnP/focal and raw-pair checks, or `--joint` to reuse startup for one production bundle without another Sync; optional `--span-percent` changes only the diagnostic trial. Joint replay records the production bundle time allowance and gives its outer cancellation guard 15 additional seconds.
...
- `tools/debug-sync/probe_lens_inputs.py` — lens eligibility report with named constraints, applied-endpoint distortion skip details, drawn/From Points line angle and residual measurements, and zero-solve mirrored-line infinite/finite gap decomposition; optional `--force-distortion-probe` bypasses only radial-coverage gates on frozen geometry, while `--fit-seconds` performs a bounded numerical trial and preserves inputs/startup/full fit (including any candidate) alongside `--out`. Never applies results or saves the source blend.
- `tools/synthetic_sync/focal_lines.py` and `verify_focal_lines_blender.py` — independent varied/reversed line-stroke and mirrored-line fixtures; native preparation, one bundle from explicit oracle geometry/pose startup (or one real Sync with `--fresh`), apply, stale-stroke and generated save/reopen certification checks. No user file is opened.
- `tools/synthetic_sync/verify_from_points_blender.py` — generated zero-solve RNA checks for stable From Points endpoint IDs, ignored legacy strokes/Known 3D links, source-change staleness and save/reopen behavior.
...
- `tools/synthetic_sync/verify_plane_groups_blender.py` — generated plane-group labels, per-axis and per-scene membership counts, disabled/default members, stable enum numbers and saved-file roundtrip; no solves.
- `tools/synthetic_sync/focal_orientation.py` and `verify_focal_orientation_blender.py` — unchanged true cameras/picks/world priors with a tilted stored anchor; oracle-start joint recovery, free-frame control, independent withheld geometry, persisted anchor calibration and stale-constraint rejection. No full Sync registration or user files.
- `tools/synthetic_sync/focal_constraint_saved_start.py` also supports unchanged-case replay, `--fixed-frame`, `--max-iterations` and `--relative-cost-tolerance` diagnostic controls. It validates archived starts, preserves endpoint/call/source records including tolerance, and never reruns Sync; iteration overrides retain the time cap.
