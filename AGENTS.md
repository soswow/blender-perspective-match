...
| Shared fixed/free focal final fit | `core/focal_bundle.py` (NumPy joint focal/pose/point/line fit; bounded runtime/size, point depth screen, conditional sensitivity; `MAX_ITERATIONS=400`, `MAX_SECONDS=60`, `MAX_DENSE_JACOBIAN_BYTES` for the bundle); `core/focal_orientation.py` (observable common-rotation basis from point/line world relations; `ORIENTATION_INDEPENDENCE_SINE`; corrected anchor orientation persists in its private calibration with identity root); `core/focal_lines.py` (four-coordinate infinite lines, stroke residuals); `core/focal_constraints.py` (point plane and shared mirror plane constraints); `core/focal_line_constraints.py` (point-supported line plane position/direction, line/line or line/world-axis parallelism, `LINE_RELATION_DIRECTION_RESIDUAL_PX`, `LINE_RELATION_DIRECTION_HARD_SINE` and `LINE_HARD_PLANE_MIN_TANGENT`; hard-plane directions stay constrained inside pixel residuals and publication). `scene.run_solve_sync` calls `refine_fixed_focals` after initialization; independent `core/lens_refine.py` frees the focal parameters in the same fit. Line planes need one independently located same-bucket member for X/Y/Z or three non-collinear members for Free; fitted points and fixed Known 3D line references can supply support. |
...
| `request.py` | Complete `SyncSolveRequest` v5, JSON capture/restore, separate evidence/seed fingerprints; v1–4 migrate without a seed |
...
**Scene Solve:** a certified current solution skips registration; cold or changed requests use the core initializer below. Every successful initializer reaches the common fixed-focal final fit. Replacement preserves supported evidence and must pass physical/constraint checks without worsening the same frozen weighted point/line/prior objective. The core initializer remains available independently to legacy lens-search trials; those searches run the scene final fit after selecting lenses.
