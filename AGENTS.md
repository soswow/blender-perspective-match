...
- `tools/synthetic_sync/focal_constraint_saved_start.py` also supports unchanged-case replay, `--fixed-frame` and `--max-iterations` diagnostic controls. It validates archived starts, preserves endpoint/call/source records, and never reruns Sync; iteration overrides retain the time cap.
