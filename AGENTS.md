...
- `tools/synthetic_sync/` — seeded Sync cases with an independent projection/visibility oracle, withheld object checks, input-order/cache replay, and generated Blender scene/save-reopen validation; `evidence.py` compares added landmarks with paired noise and equal picking cost. See `tools/synthetic_sync/README.md`; preserve exact JSON evidence when investigating a failure.
