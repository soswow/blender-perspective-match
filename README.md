...
The extension is a native port of the manual workflow from Perspective Match Studio. It does not require Electron, Node.js, a Python sidecar, PyTorch, GeoCalib, or network access. Core matching uses only NumPy (bundled with Blender). **Find AprilTags** and **Find Auto Features** ship OpenCV as a bundled wheel.
...
- Detect and match **automatic ORB/SIFT features** across sync-enabled stills into anonymous auto tracks (Metashape-style dots; multi-view vs orphan colours; runs in a background thread). Multi-view tracks can join Solve Sync at a reduced weight without flooding the personal landmark list.
...
The extension uses the NumPy bundled with Blender. **Find AprilTags** / **Find Auto Features** bundle `opencv-contrib-python-headless` as a platform wheel (extracted on install / enable). No manual `pip install` into Blender’s Python.
...
3. Add landmarks for features visible in two or more stills (≥5 shared 2D picks), **or** link **Known 3D** Blender objects (≥3) and pick them in the other stills. Each landmark keeps a stable `item_id` plus a `creation_index` (add order). The **A–Z** toggle beside the list (below + / line / import / tracking / −) is a depressed/undepressed control: on = alphabetical by name, off = original add order. Sort only changes the list display — not storage order. The **font** toggle under A–Z shows each landmark's name next to its pick on the plate (off by default). The **Duplicate** button under that copies type / On Ground / Use in Sync (and sets Pick Confidence from existing picks) but clears Known 3D links, parallel links, picks, and solved positions. Uncheck **Use in Sync** (list checkbox or detail prop) to exclude a landmark from Solve Sync / Diagnose without deleting its picks — useful when sync starts failing after adding one point. With **Landmark Empties** on, that also removes its helper from `PM_Sync_Landmarks` (and restores it when re-enabled).
   ...
   **Find Auto Features** (tracking icon next to AprilTags, or the **Auto Features** box below the landmark list): runs in a **background thread** (Esc / Cancel to stop). Detects ORB or SIFT keypoints on every sync-enabled still, matches pairs with a Lowe ratio test + fundamental/homography RANSAC, drops the worst residuals, and stores anonymous **auto tracks** — not rows in the personal landmark list. Tunables: **Max** features/still, **Ratio** (Lowe; higher = more raw matches), **RANSAC** px threshold, **Keep %** (post-RANSAC cull), **Orphans** (max unmatched dots drawn per still). Overlay: filled dots (eye on the Auto Features box); **amber** = matched in ≥2 stills, **cyan** = orphan in this still only. **Use Auto Features in Sync** (on by default) feeds multi-view tracks into Solve Sync / Diagnose at a reduced weight (~0.35). Clear removes them. Prefer a few solid manual / AprilTag / Known 3D landmarks; auto tracks densify the graph.
