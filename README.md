...
Native port of the manual workflow from Perspective Match Studio — no Electron, sidecar, or network. Core matching uses NumPy (bundled with Blender); **Find AprilTags** / **Detect VP Lines** ship OpenCV as a bundled wheel.

## Features

- Multiple match cameras per scene, each with its own still and calibration
...
No manual `pip install` into Blender’s Python. Wheels are fetched by `./scripts/fetch-wheels.sh` (also called from build / link-dev scripts).
