...
6. **Solve Sync** writes a rigid transform (`R`, `t`, scale 1) onto non-anchor root Empties — or a similarity with free scale if a rigid pose cannot lock (different private-world metrics). Use **Diagnose** first to see per-landmark RMSE without moving cameras; **Clear** resets sync transforms. **Refine Lenses** searches each unlocked match’s focal length (re-orients from VP lines at each trial, with a VP angular prior) to lower sync RMSE, then runs Solve Sync. The % field beside it is the ± search window around current fx (default 18). Skips **1-point** matches and matches without enough VP lines (Manual FOV matches are included). The eye icon on **Sync Matches** toggles landmark picks on the plate (same pattern as VP Lines); **Landmark Empties** still controls the 3D helpers after sync.
