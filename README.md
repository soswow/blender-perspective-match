...
- Solve orientation and FOV from orthogonal VPs; **Use Known 3D** polishes FOV / principal point / camera position from landmark picks without leaving Auto from VPs; **Iterate Known 3D** repeats that polish with Solve Sync until RMSE settles; manual FOV / ROS `camera_info` YAML import (including OpenCV distortion coefficients)
...
- Ground origin pick; multi-match sync via landmarks, Known 3D Empties (optional Known 3D Slack), Is in Plane buckets, Is Mirror Of pairs across one Mirror Empty, and AprilTag 25h9 / 36h10 (locked/imported K plus shared ground landmarks across 3+ images can replace VP lines)
